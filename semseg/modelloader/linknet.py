...
from semseg.modelloader.utils import conv2DBatchNormRelu, linknetUp, residualBlock


class linknet(nn.Module):

    def __init__(self, feature_scale=4, n_classes=21, is_deconv=True, in_channels=3, is_batchnorm=True, pretrained=False):
        ...
        # Encoder
        self.convbnrelu1 = conv2DBatchNormRelu(in_channels=3, kernel_size=7, out_channels=filters[0],
        ...
        self.finalconvbnrelu2 = conv2DBatchNormRelu(in_channels=32/feature_scale, kernel_size=3, out_channels=32/feature_scale, padding=1, stride=1)
    ...
    def forward(self, x):
        ...
        # print(x.data.size())
        e1 = self.encoder1(x)
        # print('e1.data.size()', e1.data.size())
        e2 = self.encoder2(e1)
        # print('e2.data.size()', e2.data.size())
        e3 = self.encoder3(e2)
        # print('e3.data.size()', e3.data.size())
        e4 = self.encoder4(e3)
        # print('e4.data.size()', e4.data.size())

        # Decoder with Skip Connections
        d4 = self.decoder4(e4)
        e3 = F.upsample_bilinear(e3, d4.size()[2:])
        d4 += e3
        d3 = self.decoder3(d4)

        e2 = F.upsample_bilinear(e2, d3.size()[2:])
        d3 += e2
        d2 = self.decoder2(d3)

        e1 = F.upsample_bilinear(e1, d2.size()[2:])
        d2 += e1
        d1 = self.decoder1(d2)
        ...
        print(x.data.size())
        print(f3.data.size())
        # f3 = F.upsample_bilinear(d2, e1.size()[2:])

        return f3

if __name__ == '__main__':
    n_classes = 21
    model = linknet(n_classes=n_classes, pretrained=False)
    # model.init_vgg16()
